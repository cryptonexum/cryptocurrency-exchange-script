...
if [ -z "$STY" ]; then
    screen -dm -S cryptonexum /bin/bash "$0"
    echo "Started setup in a screen session named cryptonexum , This process can take upto 10-20 mins. You can reattach to it with 'screen -r cryptonexum'."
    exit
fi
