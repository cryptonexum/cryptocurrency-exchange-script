...
if [[ -d "/data/wwwroot/${DOMAIN}" && -d "/data/wwwroot/cryptonexum_unpack/codebase" && -d "/data/wwwroot/cryptonexum_unpack/webserver" ]]; then
    echo "Moving files..."
    mv /data/wwwroot/cryptonexum_unpack/codebase /data/wwwroot/ || { echo "Failed to move codebase directory."; exit 1; }
    mv /data/wwwroot/cryptonexum_unpack/webserver/* /data/wwwroot/${DOMAIN}/ || { echo "Failed to move webserver files."; exit 1; }
...
sed -i "s|const DB_NAME = 'cryptonexumexchange';|const DB_NAME = '$DB_NAME';|g" "$PHP_CONFIG_FILE"
...
sed -i "s|const SHORT_NAME = 'cryptonexum';|const SHORT_NAME = '$SITE_NAME';|g" "$PHP_CONFIG_FILE"
