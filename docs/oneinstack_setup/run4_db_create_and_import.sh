...
# Read MySQL credentials for cryptonexum db and other info from /opt/credentials.yml
...
for SQL_FILE in /data/wwwroot/cryptonexum_unpack/*.sql; do
    echo "Importing $SQL_FILE into $DB_NAME..."
    $MARIADB_BIN -uroot -p"$MYSQL_ROOT_PASSWORD" "$DB_NAME" < "$SQL_FILE"
done

# Update cryptonexum_admin table
echo "Updating cryptonexum_admin table..."
$MARIADB_BIN -uroot -p"$MYSQL_ROOT_PASSWORD" "$DB_NAME" -e "
UPDATE cryptonexum_admin
...
echo "cryptonexum Database setup and updates complete. Please visit https://${DOMAIN}/install_check.php and https://${DOMAIN}/"
